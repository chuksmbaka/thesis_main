...
######################### Simulation #####################################
   ...
   # print("a: ", a)
    ...
    #plt.show()
    tran = sim.add_flux(fsrc, mp.FluxRegion(center=mp.Vector3(8), size=mp.Vector3(y=4)))
    sim.run(until=200)
    res = sim.get_eigenmode_coefficients(tran,
                                         [1],
                                         direction=mp.NO_DIRECTION,
                                         kpoint_func=lambda f,n: kpoint)
    print("flux at x = 7\u00B5m:, {:.6f}, {:.6f}".format(mp.get_fluxes(tran)[0],abs(res.alpha[0,0,0])**2))
#####################################
